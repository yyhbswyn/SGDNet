...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def forward(self, feats):
        ...
        Args:
            feats (tuple[Tensor]): Features from the upstream network, each is
                a 4D-tensor.  这里输入进来的就是3个tuple特征
        Returns:
            tuple[Tensor]: A tuple of multi-level predication map, each is a
                4D-tensor of shape (batch_size, 5+num_classes, height, width).
                返回的也是3个尺度上的预测值，（5，32+1+4，W,H）
        """

        print("feats的长度：",len(feats))
        ...
        return tuple(pred_maps),  #[4,111,8,8][4,111,16,16][4,111,32,32]
    ...
    def _get_targets_single(self, anchors, responsible_flags, gt_bboxes,
        ...
        gt_labels_one_hot = F.one_hot(
            gt_labels.to(torch.int64), num_classes=self.num_classes).float()
