...
class SampleFrames:
    ...
    Args:
        ...
        frame_interval (int): Temporal interval of adjacent sampled frames.
            Default: 1.  每隔几帧采一次 。我想让训练采样就直接按照关键帧进行采样
        num_clips (int): Number of clips to be sampled. Default: 1.
        temporal_jitter (bool): Whether to apply temporal jittering.  抖动操作。。
            Default: False.
...
class SampleAVAFrames(SampleFrames):
    ...
    def _get_clips(self, center_index, skip_offsets, shot_info):
        #将梳理出来的关键帧标号center_index=1，31，61，91，121和shot_info=(0，297)传入上面的方法中进行计算。还有一串长度为cliplen=32的随机偏移量[-1,1.5)
        start = center_index - (self.clip_len // 2) * self.frame_interval  #start=-31,-1,29,59,89
        end = center_index + ((self.clip_len + 1) // 2) * self.frame_interval #end =33,63,93,123,153
        frame_inds = list(range(start, end, self.frame_interval))  #start和end上下各一个数，分别为起止。
        #我是验证的时候出错，validate参数指定时候就已经是testmode了，所以这里没有经过frame_inds = frame_inds + skip_offsets
        if not self.test_mode:  #训练过程中要加偏移量
            frame_inds = frame_inds + skip_offsets  #简单的和上面的[-1,-1,-1,-1,0,0,-1这样的偏差量做一个计算]
        #test和val都是直接经过的下面这一步
        frame_inds = np.clip(frame_inds, shot_info[0], shot_info[1] - 1)  #前面负数全部为0，从1开始每隔2帧做一个采样。我需要做个改进，让他自行返回不同长度的frames_ids。
        return frame_inds

    def __call__(self, results):
        fps = results['fps']  #30
        timestamp = results['timestamp']   #表示当前是第几帧
        timestamp_start = results['timestamp_start'] #2
        shot_info = results['shot_info']  #（0，297.0）

        center_index = fps * (timestamp - timestamp_start) + 1  #这样正好取到的就是所有标注的关键帧
        #timestamp=2,3,4,5,6,  timestamp_start=2,center_index=1，31，61，91，121

        skip_offsets = np.random.randint(  #随机整数，范围[low,high)
            -self.frame_interval // 2, (self.frame_interval + 1) // 2,
            size=self.clip_len)
        frame_inds = self._get_clips(center_index, skip_offsets, shot_info)   #将梳理出来的关键帧标号center_index=1，31，61，91，121和shot_info=(0，297)传入上面的方法中进行计算
        start_index = results.get('start_index', 0)  #我写的start index 是1

        frame_inds = np.array(frame_inds, dtype=np.int) + start_index  #这一步会取32帧，如果不够，就填充同样的帧进去。
        results['frame_inds'] = frame_inds
        results['clip_len'] = self.clip_len
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = 1
        results['crop_quadruple'] = np.array([0, 0, 1, 1], dtype=np.float32)  #这个1/4不知道是干嘛的。。。。
        return results   #这里是有可以定位的video_id信息，results[img_key]='387,0005'
...
#这里我想改写一下它进行采样的方式。
@PIPELINES.register_module()
class SampleTITANFrames(SampleFrames):
    def __init__(self, clip_len, frame_interval=2, test_mode=False):
        super().__init__(clip_len, frame_interval, test_mode=test_mode)
    ...
    def _get_clips(self, center_index, skip_offsets, shot_info):
        ...
        #     frame_inds = frame_inds + skip_offsets  # 简单的和上面的[-1,-1,-1,-1,0,0,-1这样的偏差量做一个计算]
        # # test和val都是直接经过的下面这一步
        frame_inds = np.clip(center_index, shot_info[0],
                             shot_info[1] - 1)  # 前面负数全部为0，从1开始每隔2帧做一个采样。我需要做个改进，让他自行返回不同长度的frames_ids。
        return frame_inds
    ...
    def __call__(self, results):
        ...
        skip_offsets = np.random.randint(  # 随机整数，范围[low,high)
            -self.frame_interval // 2, (self.frame_interval + 1) // 2,
            size=self.clip_len)
        frame_inds = self._get_clips(center_index, skip_offsets,
                                     shot_info)  # 将梳理出来的关键帧标号center_index=1，31，61，91，121和shot_info=(0，297)传入上面的方法中进行计算
        start_index = results.get('start_index', 0)  # 我写的start index 是1

        # frame_inds = np.array(frame_inds, dtype=np.int) + start_index  # 这一步会取32帧，如果不够，就填充同样的帧进去。
        frame_inds = np.array(frame_inds, dtype=np.int) # 这一步会取32帧，如果不够，就填充同样的帧进去。
        results['frame_inds'] = frame_inds
        results['clip_len'] = self.clip_len
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = 1
