    ...
    def forward(self, x):
        ...
        Returns:
            tuple[torch.Tensor]: The feature of the input samples extracted
                by the backbone.
        """
        # print("输出一下进入模型的时候的x的形状：",x.shape)     #[5,3,64,256,256]
        x_slow = nn.functional.interpolate(
            x,
            mode='nearest',
            scale_factor=(1.0 / self.resample_rate, 1.0, 1.0))  #slow数组采样结果[1,3,1,256,256]
        x_slow = self.slow_path.conv1(x_slow)   #[1,64,1,128,128]
        x_slow = self.slow_path.maxpool(x_slow) #[1,64,1,64,64]
        # print("输出简单进行conv1和maxpool的x_slow的形状：",x_slow.shape)

        x_fast = nn.functional.interpolate(
            x,
            mode='nearest',
            scale_factor=(1.0 / (self.resample_rate // self.speed_ratio), 1.0,
                          1.0))   #fast数组采样结果[1,3,10,256,256]这个和输入的x形状一样
        x_fast = self.fast_path.conv1(x_fast)   #[1,8,10,128,128]
        x_fast = self.fast_path.maxpool(x_fast)  #[1,8,10,64,64]
        # 此时slow[1,64,1,64,64] fast[1,8,10,64,64]

        if self.slow_path.lateral:
            x_fast_lateral = self.slow_path.conv1_lateral(x_fast)  #[1,16,2,64,64]
            x_slow = torch.cat((x_slow, x_fast_lateral), dim=1)  #[1,80,1,64,64]

        for i, layer_name in enumerate(self.slow_path.res_layers):
            res_layer = getattr(self.slow_path, layer_name)
            x_slow = res_layer(x_slow) #[1,256,1,64,64]
            res_layer_fast = getattr(self.fast_path, layer_name)
            x_fast = res_layer_fast(x_fast) #[1,32,10,64,64]
            if (i != len(self.slow_path.res_layers) - 1
                ...
                x_fast_lateral = conv_lateral(x_fast)    #[1,64,2,64,64]
                x_slow = torch.cat((x_slow, x_fast_lateral), dim=1) #[1,320,1,64,64]
        #
        # print("out前的slow:",x_slow.shape)   #[5,2048,8,16,16]    如果是sampletitanframes的话，就是slow[1,2048,1,16,16] fast[1,256,10,16,16]
        # print("out前的fast:",x_fast.shape)   #[5,256, 64,16,16]
        out = (x_slow, x_fast)     #他就是一个长度为2的元组tuple,0是slow,1是fast

        assert len(out)==2
        # print("slow:",x_slow.shape)  #[10,2048,6,48,48]
        # print("fast:",x_fast.shape)  #[10, 256, 48, 16, 16]
        return out
