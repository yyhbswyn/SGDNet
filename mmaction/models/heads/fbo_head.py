...
class FBOHead(nn.Module):
    ...
    def forward(self, x, rois, img_metas, **kwargs):  #x[5,2048,1,8,8], rois=[5,5]
        # [N, C, 1, 1, 1]  s就是short-term features, l就是long-term features
        st_feat = self.temporal_pool(x)  #[5,2048,1,8,8]
        st_feat = self.spatial_pool(st_feat)  #[5,2048,1,1,1]
        identity = st_feat

        # [N, C, window_size * num_feat_per_step, 1, 1]  Sample long-term features for each ROI feature
        lt_feat = self.sample_lfb(rois, img_metas).to(st_feat.device)  #[5,2048,600,1,1]

        fbo_feat = self.fbo(st_feat, lt_feat)  #长期短期经过fbo算子之后，获得fbo_feat [5,512,1,1,1]

        out = torch.cat([identity, fbo_feat], dim=1)   #[5,2560,1,1,1]
        return out   #long term feature
